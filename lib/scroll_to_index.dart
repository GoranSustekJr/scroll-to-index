...
abstract class AutoScrollController implements ScrollController {
  ...
  Future scrollToIndex(int index,
      {Duration duration = scrollAnimationDuration, double? preferPosition});
...
mixin AutoScrollControllerMixin on ScrollController
  ...
  Future _bringIntoViewportIfNeed(
      int index, double? preferPosition, Future move(double offset)) async {
  ...
  double _positionToAlignment(double position) {
    return position;
  }

  double _alignmentToPosition(double alignment) => alignment;
